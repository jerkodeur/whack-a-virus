...
class Game extends React.Component {
  ...
  componentDidMount() {
    ...
    axios.get(url).then((res) => {
      this.setState({
        image: res.data.hits[6].largeImageURL,
      });
    });
  }
  ...
  handleClick = (e) => {
    ...
    id === "circle" ? this.increaseHealth() : this.decreaseHealth();
  };
  permanentDecrease = () => {
    this.state.health < 100 && this.setState({ health: this.state.health + 1 });
    this.state.health === 100 && this.setState({ win: "lose" });
  };
  ...
  render() {
    return (
      <div className="containerGame">
        ...
        <button id="start" onClick={this.handle}>
          Start
        </button>
      </div>
    );
